...
exports.processPayment = async (req, res) => {
  const { wholesellerId, orderId, totalAmount } = req.body;

  try {
    // Implement your payment processing logic here
    console.log('Processing payment for:', wholesellerId, orderId, totalAmount);

    // Assuming payment was successful, create a notification directly in the wholeseller's document
    const notification = {
      message: "Order Success",
      icon: "success_icon.png", // Adjust this path as needed
      orderId,
      amount: totalAmount,
      dateTime: new Date(),
    };

    // Find the wholeseller by ID and push the notification to the notifications array
    const result = await Wholeseller.findByIdAndUpdate(
      wholesellerId,
    ...
    res.status(200).json({ success: true, message: "Payment processed and notification created successfully" });
  } catch (error) {
